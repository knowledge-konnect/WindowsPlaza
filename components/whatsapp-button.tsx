...
const href = buildStaticWhatsAppUrl(WHATSAPP_CONFIG.floatingButtonMessage)
...
export function WhatsAppButton() {
  return (
    <Link
      ...
      className="fixed bottom-5 right-5 z-50 hidden h-14 w-14 items-center justify-center rounded-full shadow-md shadow-black/25 transition-all hover:scale-105 hover:shadow-lg hover:shadow-black/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 lg:flex"
      style={{ backgroundColor: "#25D366" }}
      aria-label={`Chat with ${BUSINESS_INFO.brandName} on WhatsApp for instant uPVC window quotes in ${BUSINESS_INFO.location.city}`}
    >
